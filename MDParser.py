...
class MDParser:
    ...
    def write_index_file(self, contents, directory_path):
        ...
        with open(f"{directory_path}/index.md", "w") as target:
            ...
            target.write("</p>")
            target.write("\n\n")
            target.write("## Helpful Subjects\n")
            target.write("[How to perform an upgrade](upgrade.md)")
            target.write("\n\n")
        ...
        for key, value in sorted_dict.items():
            ...
            if self.can_print_key(value, path):
                ...
                if isinstance(value, list):
                    self.print_list(value, indent+3, path, file_handle)
                elif isinstance(value,dict):
                    if "0" in value.keys():
                        self.print_list(value["0"], indent+3, path, file_handle)
                        value.pop("0")
                    self.write_nested_index(value, file_handle, indent+3, path)
            path = path[:path.find(f"{key}/")]

    def print_list(self, input_list, indent, path, file_handle):
        pad = " "*indent
